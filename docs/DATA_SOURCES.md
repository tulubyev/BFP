...
- **Маска постоянных источников тепла** (`backend/services/firmsHistory/staticSources.ts`, метод
  `static-mask-v1`): газовые факелы и промышленность на севере Иркутской области дают термоточки
  в одном и том же месте изо дня в день. Маска — объединение двух правил: по **нашей истории**
  `gis.fire_hotspots` (ниже) и по **ячейкам архива FIRMS** с `type = 2` (раздел «NASA FIRMS —
  годовой архив по России»): точка в ячейке из `firms-static-cells.<год>.json` статична сразу, без
  ожидания истории. Нет файла — работает только правило по истории. Место — статический
  источник, если в радиусе 500 м (≈ 1,3 пикселя VIIRS) термоточки были в **≥ 4 разных дня (UTC)
  за последние 14 дней** и между первым и последним из этих дней **≥ 7 дней**. Правило разброса
  не даёт замаскировать пожар, который 4–5 дней подряд тлеет на одном месте; факел возвращается
  неделя за неделей. SQL агрегирует точки в ячейки ~445 м (различные дни на ячейку, ячейки с
  ≥ 2 днями), дни соседних ячеек в радиусе объединяются в коде.
  - Точки в статических местах не создают и не продлевают инциденты. FIRMS-инцидент, все точки
    которого в статических местах, получает `metadata.status = 'static_source'` и
    `metadata.static_mask` (правило и время пометки); строка не удаляется, пометка не снимается,
    строки без `metadata.method` не трогаются. Инцидент с точками и вне маски остаётся как есть.
  - `GET /api/monitoring/forest-changes` и главная по умолчанию их скрывают;
    `?static_sources=include` — показать вместе с остальными, `?static_sources=only` — только их
    (на странице «Инциденты» — фильтр «Постоянные источники тепла»). Карточка: «Постоянный
    источник тепла (факел/промышленность)».
  - Журнал `firms_history`: `masked.hotspots` (точки в окне 72 ч, исключённые из кластеризации),
    `masked.incidents` (инциденты, помеченные этим запуском), `masked.staticLocations`.
  - Пока истории меньше 8 дней (запись идёт с 2026-09-25), правило по истории не маскирует ничего;
    ячейки архива действуют с первого запуска. Инцидент, помеченный при наличии архива, получает
    `static_mask.archive_version` (год файла ячеек). В логе задачи — `N locations + M archive cells`.
    Слой термоточек FIRMS 24 ч на карте маска не фильтрует.

## NASA FIRMS — годовой архив по России (задача #12, часть A)

- Источник: NASA FIRMS, VIIRS S-NPP 375 м, годовые CSV по стране, без ключа:
  `https://firms.modaps.eosdis.nasa.gov/data/country/viirs-snpp/<год>/viirs-snpp_<год>_Russian_Federation.csv`
  (~70 МБ/год). Лицензия — открытые данные NASA, атрибуция «NASA FIRMS»
  (https://earthdata.nasa.gov/firms). Это стандартный (не NRT) продукт; архив обновляется **раз в
  год**, файл за год появляется через несколько месяцев после его окончания (в 2026-09 последний —
  2024). Один спутник (S-NPP), поэтому с нашей историей NRT (3 спутника, с 2026-09-25) **не
  сравнивать**.
- Поле `type`: `0` — предполагаемый пожар растительности, `1` — вулкан, `2` — прочий статичный
  наземный источник (газовые факелы, промышленность), `3` — шельф (платформы). Вулканы в итоги не
  входят.
- Сборка: `bash scripts/regional/build-firms-archive.sh [work-dir]` (Docker, ~0,5 ГБ диска, годы
  `FIRST=2019 LAST=2024` по умолчанию; ~1 мин на 3–4 млн строк). Скачивает CSV, потоково относит
  точки к 83 субъектам из новейшего `ru-regions.<версия>.geojson` (bbox-префильтр +
  point-in-polygon, `scripts/regional/aggregate.ts`); точки вне 83 субъектов (Крым, море)
  отбрасываются. Проверки: 83 субъекта, каждый год не пуст, каждый файл ≤ 300 КБ gzip; иначе
  `FAIL` и ничего не пишется. Результат из `<work-dir>/out/` скопировать в
  `frontend/public/data/regional/` и выпустить обычным PR.
- `firms-archive.<последний год>.json`:
  `{ version, generatedAt, source: { name, urls, license }, years: [2019, …],
  regions: { "RU-IRK": { name, areaKm2, years: { "2024": { vegetation, static, offshore, frpVegetation } } } } }` —
  число детекций (не пожаров) по типам и сумма FRP (МВт) детекций растительности; `areaKm2` —
  площадь упрощённого полигона OSM (`@turf/area`, с территориальным морем у приморских субъектов).
- `firms-static-cells.<последний год>.json`: `{ version, generatedAt, gridDeg: 0.004, years: [2022, 2023, 2024],
  cells: [[row, col], …] }` — ячейки сетки маски (`cellOf()` в `staticSources.ts`: `row = floor(lat / 0.004)`,
  столбец масштабирован по cos широты), где за последние три года архива была точка `type = 2`.
- Обновление: когда NASA выпустит следующий год — `LAST=2025 bash scripts/regional/build-firms-archive.sh`,
